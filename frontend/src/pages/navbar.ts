...
export function renderNavbar(): string {
	...
	const avatarUrl = avatar && avatar.startsWith('http') ? avatar : `https://${hostIP}:3443/api${avatar}`;

	const linkClass = (hash: string) =>
		`px-4 py-2 rounded transition-colors duration-200 ease-in-out ${
			currentHash === hash ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700'
		}`;
		...
		<nav class="bg-gradient-to-r from-gray-800 to-gray-900 shadow-lg p-4 flex flex-wrap items-center justify-between">
			<div class="flex items-center space-x-4">
				...
				<div id="navLinks" class="hidden md:flex space-x-4">
					...
					<a href="#/friends" class="${linkClass('#/friends')}">Friends</a>
					<a href="#/tournaments" class="${linkClass('#/tournaments')}">Tournaments</a>
				</div>
			</div>
			...
			<div class="relative">
				...
				>
					...
					<div class="ml-2 text-left hidden md:block">
						<p class="text-sm font-medium text-white">${escapeHtml(alias || username)}</p>
						<p class="text-xs text-gray-400">@${escapeHtml(username)}</p>
					</div>
				</button>
