...
export function renderSettings(): string {
	...
	if (!user) return `<p class="text-center mt-10 text-red-500">No user data.</p>`;
		...
		<section class="max-w-2xl mx-auto mt-10 p-6 bg-gray-800 text-white rounded-lg shadow-lg animate-fadeIn">
			...
			<form id="settingsForm" class="space-y-4">
				<div>
					...
					<input type="text" name="username" value="${escapeHtml(user.username)}" class="w-full px-4 py-2 rounded bg-gray-900 text-white border border-gray-600">
				</div>
				<div>
					<label class="block mb-1 text-gray-300">Alias</label>
					<input type="text" name="alias" value="${escapeHtml(user.alias)}" class="w-full px-4 py-2 rounded bg-gray-900 text-white border border-gray-600">
				</div>
				<div>
					<label class="block mb-1 text-gray-300">Email</label>
					<input type="email" name="email" value="${escapeHtml(user.email)}" class="w-full px-4 py-2 rounded bg-gray-900 text-white border border-gray-600">
				</div>
				<div>
					<label class="block mb-1 text-gray-300">Avatar</label>
					<input id="avatarFile" type="file" accept="image/*" class="block w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700">
					<img id="avatarPreview" src="" alt="Avatar preview" class="mt-2 h-16 rounded" />
